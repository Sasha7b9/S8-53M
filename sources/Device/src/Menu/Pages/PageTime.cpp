...
void PageTime::OnChanged_PeakDet(bool active)
{
    if (active)
    {
        ...
        if (PEAKDET_IS_DISABLE)
        {
            ...
            FPGA::WriteToHardware(WR_ADD_RSHIFT_DAC1, (uint8)shift[0][BALANCE_ADC_TYPE], false); //-V2563
            FPGA::WriteToHardware(WR_ADD_RSHIFT_DAC2, (uint8)shift[1][BALANCE_ADC_TYPE], false); //-V2563
        }
        else
        {
            FPGA::WriteToHardware(WR_ADD_RSHIFT_DAC1, 3, false);     // Почему-то при пиковом детекторе смещение появляется. Вот его и компенсируем. //-V2563
            FPGA::WriteToHardware(WR_ADD_RSHIFT_DAC2, 3, false); //-V2563
        }
        if (PEAKDET_IS_DISABLE)
        {
