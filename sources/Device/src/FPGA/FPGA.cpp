...
void FPGA::State::Restore()
{
    ...
    for (int ch = 0; ch < 2; ch++)
    {
        for (int mode = 0; mode < 2; mode++)
        {
            ...
            {
                rShiftAdd[ch][range][mode] = set.chan[ch].rshift_add[range][mode];
            }
        }
    }
    ...
    for (int ch = 0; ch < 2; ch++)
    {
        for (int mode = 0; mode < 2; mode++)
        {
            for (int range = 0; range < Range::Count; range++)
            {
                set.chan[ch].rshift_add[range][mode] = rShiftAdd[ch][range][mode];
            }
        }
    }
...
//    LOG_WRITE("%s : %d", __FUNCTION__, data);
    ...
    else if (type == TypeWriteAnalog::ChanParam1 && IS_SHOW_REG_PARAM_B)
    {
        ...
        (IS_SHOW_REG_TRIGPARAM || IS_SHOW_REG_RANGE_A || IS_SHOW_REG_RANGE_B || IS_SHOW_REG_PARAM_A || IS_SHOW_REG_PARAM_B))
    {
...
{
    return set.memory.enum_points_fpga.value;
}
