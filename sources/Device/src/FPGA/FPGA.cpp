...
void FPGA::BUS::Write(uint16 * const address, uint16 data, bool restart)
{
    ...
    led_Trig.Disable();                 // После каждой засылки выключаем лампочку синхронизации
}
...
void FPGA::State::Restore()
{
    ...
    for (int ch = 0; ch < 2; ch++)
    {
        for (int mode = 0; mode < 2; mode++)
        {
            ...
            {
                rShiftAdd[ch][range][mode] = setNRST.chan[ch].rshift_auto[range][mode];
            }
        }
    }
    ...
    for (int ch = 0; ch < 2; ch++)
    {
        for (int mode = 0; mode < 2; mode++)
        {
            for (int range = 0; range < Range::Count; range++)
            {
                setNRST.chan[ch].rshift_auto[range][mode] = rShiftAdd[ch][range][mode];
            }
        }
    }
