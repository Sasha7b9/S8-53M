...
// Минимальное значение смещения канала по напряжению, засылаемое в аналоговую часть. Соответствует смещению 10 клеток
// вниз от центральной линии
#define RShiftMin           100
// Среднее значение смещения канала по напряжению, засылаемое в аналоговую часть. Соответствует расположению марера по
// центру экрана
#define RShiftZero          32500
// Максимальное значение смещения канала по напряжению, засылаемое в аналоговую часть. Соответствует смещению 10 клеток
// вверх от центральной лиини
...
#define STEP_RSHIFT ((RSHIFT_IN_CELL) / Grid::DELTA)
...
// Нулевое значение уровня синхронизации, засылаемое в прибор. Маркер синхронизации при этом находитися на одном уровне
// с маркером смещения по напряжению
...
// Это значение входного сигнала, считанное с АЦП, соответствует центру сетки. Если значение == 0, значит, его нет. Это
// нужно для режимов рандомизатора и поточечного вывода p2p, а также для tShift ранее считанного сигнала
...
// Если значение == 0, значит, его нет. Это нужно для режимов рандомизатора и поточечного вывода p2p, а также для tShift
// ранее считанного сигнала
#define NONE_VALUE  0
...
#define WR_START        ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x00 << 1)))  // Запуск цикла измерения. Для запуска нужно
                                                                        // записать 1
#define WR_RAZV         ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x01 << 1)))
...
#define WR_UPR          ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x05 << 1)))
...
#define WR_ADDR_NSTOP   ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x06 << 1)))
#define WR_FL_RESET     ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x08 << 1)))
#define WR_RESET        ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x1f << 1)))

#define RD_ADC_A        ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x00 << 1)))
#define RD_ADC_B        ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x02 << 1)))

#define RD_ADDR_NSTOP   ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x08 << 1)))
/*
...
#define WR_FREQ_METER_PARAMS    ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x0a << 1)))

#define RD_FREQ_LOW             ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x10 << 1)))
#define RD_FREQ_HI              ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x18 << 1)))
#define RD_PERIOD_LOW           ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x20 << 1)))
#define RD_PERIOD_HI            ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x28 << 1)))

#define RD_FL                   ((uint16 *)(HAL_FMC::ADDR_FPGA + (0x30 << 1)))  
