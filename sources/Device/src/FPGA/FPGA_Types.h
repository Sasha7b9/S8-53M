...
#define TrigLevZero RShift::ZERO // Ќулевое значение уровн€ синхронизации, засылаемое в прибор. ћаркер синхронизации при
                                // этом находитис€ на одном уровне с маркером смещени€ по напр€жению.
#define TShiftMax   16000       // ћаксимально значение задержки по времени относительно точки синхронизации, засылаемое
                                //в альтеру

// Ёто значение входного сигнала, считанное с ј÷ѕ, соответствует нижней границе сетки (-5 клеток от центра). ≈сли
// значение == 0, значит, его нет. Ёто нужно дл€ режимов рандомизатора и поточечного вывода p2p, а также дл€ tShift
// ранее считанного сигнала.
#define MIN_VALUE   27
#define AVE_VALUE   127         // Ёто значение входного сигнала, считанное с ј÷ѕ, соответствует центру сетки. ≈сли
                                // значение == 0, значит, его нет. Ёто нужно дл€ режимов рандомизатора и поточечного
                                // вывода p2p, а также дл€ tShift ранее считанного сигнала.
#define MAX_VALUE   227         // Ёто значение входного сигнала, считанное с ј÷ѕ, соответствует верхней границе сетки
                                // (+5 клеток от центра). ≈сли значение == 0, значит, его нет. Ёто нужно дл€ режимов
                                // рандомизатора и поточечного вывода p2p, а также дл€ tShift ранее считанного сигнала.
#define MIN_VALUE_SCREEN 0
#define MAX_VALUE_SCREEN 200

#define MIN_TBASE_PEC_DEAT  TBase::_500ns     // ћинимальный масштаб по времени, при котором возможно включение режима
                                              // пикового детектора.
