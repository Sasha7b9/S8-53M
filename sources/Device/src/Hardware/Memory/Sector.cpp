...
const DataReading *PacketROM::UnPack() const
{
    if (!IsValid() || !IsData())
    {
        return nullptr;
    ...
    return const_cast<DataReading *>
        (reinterpret_cast<const DataReading *>(reinterpret_cast<const uint8 *>(this) + sizeof(PacketROM)));
}
...
const DataReading *Sector::ReadData(uint numInROM) const
{
...
void Sector::GetDataInfo(bool existData[ROM::Data::MAX_NUM_SAVED_WAVES]) const
{
    ...
    while (packet && !packet->IsFree())
    {
        if (packet->IsData())
        {
            const DataReading *ds = packet->UnPack();
            if (ds)
            {
...
{
    const DataReading *dr = packet->UnPack();
