...
struct SectorSet
{
    ...
    // Возвращает указатель на ближайшую к record пустую запись (такую, в которую можно записать настройки).
    // Если таковой не имеется, возвращает nullptr
...
public:
    ...
    // Возвращает true, если в записи что-то записано (первые байты не равны 0xFFFFFFFF и 0x00000000).
    // Без учёта контрольных сумм
    bool IsSaved();

    // Стереть запись. Заполняет запись нулями. Это нужно делать с повреждёнными записями
    // (с теми, в которых не совпадают контрольные суммы)
    void Erase();
